...
### 0. その修正が本当に端末に入っているかを最初に確かめる

**ここを飛ばすと、直したはずの版ではなく古い版を何度でもテストすることになる。**
2026-09-04 に実際にこれで1周無駄にした（Codemagic で `rn-android` を再実行しただけで
...
3. 端末で **Playストア → 右上のアイコン → アプリとデバイスの管理 → 利用可能なアップデート**
   → 更新（**自動では降ってこない**）
4. 端末の **設定 → アプリ → BodyLog → アプリの詳細情報** で **versionCode が前回より大きい**
   ことを確認する。同じなら更新が届いていないので 3 に戻る

versionCode は `codemagic.yaml` が `PROJECT_BUILD_NUMBER` を app.json に注入するので、
ビルドするたびに必ず増える（app.json に書いてある `1` はローカル用の既定値で、CIでは使われない）。
