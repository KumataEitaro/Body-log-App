...
3. **1年分の記録を自動生成する**（手入力は不要）
   ...
   - 冒頭の `demo_email` を、2で作ったメールアドレスに書き換える
     （ファイル末尾の確認クエリにも同じアドレスが入っているので、まとめて置換すると楽です）
   - 全文をコピーしてSQL Editorに貼り付け、**RUN**
   - 実行後に確認クエリの結果が出る（日次サマリー約350件・記録約2,500件）
4. アプリを再読み込みして、グラフや履歴が入っていることを確認
5. 終わったら自分のアカウントに戻す

> 生成されるのは、88kg→77kg前後まで1年かけて減量した記録です。停滞期が2回・年末年始の増加・記録が飛んだ日・週末の外食と飲酒・自己ベスト更新まで入っているので、グラフも分析カードも自然に埋まります。**今日のぶんは朝と昼だけ**入っていて夜は空です（スクショを撮る日が「1日ぜんぶ埋まっている」状態だと逆に不自然なため）。

> 何度実行しても同じ結果になります（乱数を固定してあります）。やり直したくなったらもう一度RUNするだけでOKです。

> メールアドレスとパスワードは STEP 5 で使うのでメモしておいてください。

---

## STEP 3-b. 体の写真だけは実機で入れる（5分・スクショを撮るなら必須）

SQLで入れられないものが1つだけあります。**体の写真**です。
`body_photos` テーブルには写真の「置き場所（パス）」しか入っておらず、画像そのものは
Supabase Storage の `body-photos` バケットにあります。SQLからは画像ファイルを作れません。

**行だけ入れたらどうなるか**: アプリは落ちません（概要タブの「体の写真」カードに日付と
体脂肪率は出て、画像のところが灰色の空枠になります）。ただし**スクショには使えない**ので、
`seed-demo.sql` では既定でOFFにしてあります（冒頭の `seed_body_photos := false`）。

**やること（実機で）**
1. 審査用アカウントでログインした状態で、**概要タブ → 「体の写真」→ 撮影**
2. 正面・自然光で4〜5枚撮る（同じ壁・同じ距離だと比較ビューがきれいに見えます）
3. 撮るたびに体脂肪率を聞かれるので、少しずつ減る数字を入れる（例: 20.5 → 19.6 → 18.8 → 17.9 → 17.4）
4. **3枚以上**入れると、カード下の横並びの履歴も出ます（2枚以下だと出ません）

> 撮影日はその日になります。「前回から7日以上」だと撮影をうながす一文が出るので、
> スクショ用にはいちばん新しい1枚を**当日か前日**に撮っておくのがおすすめです。

**そのほか、SQLで入れていないもの**

| 入れていないもの | 理由 |
|---|---|
| `body_photos`（画像） | 上記のとおりStorageに実体が要る |
| `cycle_logs`（生理周期） | デモは35歳男性。設定でONにした人だけの機能なので入れない |
| ヘルスケア（HealthKit）連携 | 実機でしか許可できない。必要なら設定→ヘルスケア連携をONにして取り込む |
| AI相談の会話履歴 | 端末内（AsyncStorage）に保存されるためDBには無い。スクショを撮る前に相談タブで1〜2往復しておく |
