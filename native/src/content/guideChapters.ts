...
export const GUIDE_CHAPTERS = (): GuideChapter[] => [
  {
    ...
    steps: [
      ...
      // 'dock' は旧・入力ドックの照射キー。2026-09-02 の入力再設計で右下の＋ボタンが引き継いだ（components/PlusFab.tsx）
      { kind: 'spot', route: '/log', target: 'dock', title: t('文字だけで、カロリーとPFCが出る'),
        text: t('右下の＋ → 食事 → テキストで入力。「バナナと卵2個」と打って↑。写真がなくても、この一言だけでAIがカロリー・たんぱく質・脂質・炭水化物まで計算します。これがこのアプリのいちばんの発明です。量があいまいなら、AIのほうから聞き返してくれます。') },
      { kind: 'sketch', art: 'tray', title: t('保存はあなたが決める'),
        text: t('AIの結果はまずトレイに載ります。違う品目は×で消して、✓で保存。勝手に記録されることはありません。') },
      { kind: 'spot', route: '/log', target: 'dock', title: t('カメラは1つで2役'),
        text: t('＋ → 食事 → 撮影する。料理を撮れば見た目から推定、パッケージ裏の栄養成分表示を撮れば表記どおりの正確な数値。同じカメラでAIが読み分けます。') },
  ...
  },
  {
    ...
    steps: [
      ...
      { kind: 'spot', route: '/log', target: 'dock', title: t('入力シートの上にも残量'),
        text: t('＋から開く入力シートの上にも「追加後の残り」が出ます。保存する前に結果が分かるから、量の調整はトレイの上でできます。') },
  ...
  },
  {
    ...
    steps: [
      ...
      { kind: 'spot', route: '/changes', target: 'chart', title: t('体の変化は文章で'),
        text: t('概要タブは4つの大項目。「からだの分析」を開くと、体重のトレンドを「3週間で下向き」のような文章で教えます。グラフを読む力はいりません。') },
  ...
  },
  {
    ...
    steps: [
      ...
      { kind: 'spot', route: '/changes', target: 'gear', title: t('設定はここ'),
        text: t('概要タブのいちばん上に「設定」があります。通知センター・目標設定・テーマ12色・11言語・単位・ヘルスケア連携も、この行からいつでも。実績はいちばん下です。') },
    ],
  },
];
