...
export const GUIDE_CHAPTERS = (): GuideChapter[] => [
  {
    ...
    steps: [
      ...
      { kind: 'spot', route: '/log', target: 'dock', title: t('つぶやくだけでいい'),
        text: t('「バナナと卵2個」。これだけで大丈夫。↑を押すとAIが栄養を計算します。量があいまいなら、AIのほうから聞き返してくれます。') },
      { kind: 'sketch', art: 'tray', title: t('保存はあなたが決める'),
        text: t('AIの結果はまずトレイに載ります。違う品目は×で消して、✓で保存。勝手に記録されることはありません。') },
      { kind: 'spot', route: '/log', target: 'dockCamera', title: t('写真を撮るだけでもいい'),
        text: t('文字にしなくても、料理を撮って↑で送るだけ。一度に4枚まで、食べながらの連投もOKです。') },
      { kind: 'spot', route: '/log', target: 'dockLabel', title: t('成分表示がいちばん正確'),
        text: t('コンビニ食やお菓子は、パッケージ裏の栄養成分表示をこのボタンで撮ると表記どおりの数値で記録できます。長押しでバーコード読み取りも。') },
      { kind: 'sketch', art: 'myfood', title: t('マイ食品は1タップ'),
        text: t('よく食べるものは登録しておくと、タップでトレイへ・長押しで即記録。AIを待つ時間すらいりません。') },
  ...
  },
  {
    ...
    steps: [
      { kind: 'spot', route: '/training', target: 'liftInput', title: t('今日のトレーニングを記録'),
        text: t('種目を選んで、重量はダイアルをくるくる。前回の重量が最初から出ているので、思い出す手間がありません。') },
      ...
      { kind: 'spot', route: '/training', target: 'restTimer', title: t('レストタイマー'),
        text: t('筋トレを保存すると自動で走り出します。0秒でブルッと震えて「次のセットへ！」。長さは7段階から選べます。') },
