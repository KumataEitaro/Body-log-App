...
// 数量＋単位（例: 80g / 150ml / 2個 / 1杯 / 1/2枚 / 一切れ / 大さじ2）
// 分数（1/2）にも対応。漢数字（一〜十・半）は「八宝菜」「十六穀米」を壊さないよう、
// 単位が続く場合だけ数量とみなす
const QTY = /(?:[0-9０-９]+(?:[./／][0-9０-９]+)?\s*(?:g|ｇ|kg|mg|ml|ｍｌ|l|cc|個|コ|本|杯|切れ|枚|片|房|粒|尾|匹|人前|皿|袋|パック|缶|缸|カップ|さじ|膳|玉|株|束|丁|合|斤)?|[一二三四五六七八九十半]+\s*(?:個|コ|本|杯|切れ|枚|片|房|粒|尾|匹|人前|皿|袋|パック|缶|カップ|さじ|膳|玉|株|束|丁|合|斤))/gi;
// 数量を表す副詞（数字を伴わないもの）
const QTY_WORDS = /(?:ひとつかみ|ひとつまみ|一つかみ|一つまみ|少々|少量|多め|軽く|山盛り|大盛り|小盛り|適量|大さじ|小さじ|カップ|半分|約)/g;
// 括弧とその中身（全角・半角）
const PARENS = /[（(][^）)]*[）)]/g;
