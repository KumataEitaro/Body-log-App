...
// 【ダンベル種目＝片側入力】side=true のときホイールの kg は「片側（片手ぶん）」。
//   決定ボタンは「片側20kg × 8回で決定」、補足に「両側で計 40kg」を出す（lib/liftLog.ts の方針）。
import { useState } from 'react';
import { Wheel, DialSheet, WheelUnit } from '@/components/Wheel';
import { totalKg, type LiftEntry } from '@/lib/liftLog';
...
export default function SetDial({ name, bw, side = false, initialKg, initialReps, bodyWeight, subtitle, okLabel, onClose, onPick }: {
  name: string;
  /** 自重種目か（加重/補助の1本ホイールになる） */
  bw: boolean;
  /** ダンベル種目か（kg を片側の重さとして受け、両側の合計を補足に出す） */
  side?: boolean;
  initialKg: number;
  initialReps: number;
  ...
  const words = { bw: t('自重'), plus: t('加重'), assist: t('補助'), side: (w: string) => t('片側{w}', { w }) };
  const kind = loadKind(kg, bw);
  // 片側入力は加重（+）と重さそのもの（abs）にだけ付く（自重のみ・補助には重さが無い／ダンベルでない）
  const perSide = side && (kind === 'abs' || kind === 'plus');
  const entry: LiftEntry = {
    name, kg, reps: 1, sets: 1,
    mode: !bw ? 'abs' : kind === 'assist' ? 'minus' : kind === 'plus' ? 'plus' : 'bw',
    ...(perSide ? { side: true } : {}),
  };

  // 補足: 自重種目は「実負荷 約Nkg（体重の内訳）」。体重が無いときはその旨だけ。
  //       ダンベル種目は「両側で計 Nkg」（入れた数字は片側だと、決める前に分かるように）
  let hint: string | undefined;
  if (bw) {
    if (bodyWeight && bodyWeight > 0) {
      const load = totalKg(entry, bodyWeight);
      ...
      hint = kind === 'assist'
        ...
        : kind === 'plus'
          ? t('実負荷 約{n}kg（{body} + 加重{a}kg）', { n: load, body, a: perSide ? kg * 2 : kg })
          : t('実負荷 約{n}kg（{body}）', { n: load, body });
    } else {
      hint = t('体重を記録すると実負荷（体重 ± kg）が出ます');
    }
  } else if (perSide && kg > 0) {
    hint = t('両側で計 {n}kg（片側 {kg}kg × 2）', { n: totalKg(entry, null), kg });
  }

  const label = loadLabel(kg, bw, words, perSide);
  return (
    <DialSheet
