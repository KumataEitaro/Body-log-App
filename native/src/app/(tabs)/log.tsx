    ...
    <View style={{ flex: 1, backgroundColor: C.bg }}>
      ...
      >
        ...
        {/* 無料プラン向けバナー広告（課金有効ビルド×無料プランのときだけ高さが生まれる）。
            ...
            - ヒーローカードの**外**に置く: 司令塔行（解釈1行＋CTA「何を食べる？」）はヒーロー内に
              あり、そこへ広告が割り込むと「数字→解釈→行動」の1画面が切れる。カードの外・直後なら
              残量を読み終えた区切りに入るので、読みの流れを断たない
            - カード調停（lib/logCards.ts）の対象カード群（注意喚起・朝の1問・昨日の穴埋め・
              チェックリスト・気分・ポジティブ）より**上**＝この画面でいちばん見られる位置。
              調停対象が0枚の日でも位置が動かない（広告の場所が日替わりで動くと不信感になる）
            - 記録ボタン（右下＋・司令塔CTA）の直上には置かない＝誤タップにならない
            - 1画面1枠の原則は維持（placement='log' のまま・食事タブに他の枠は無い） */}
        <AdBanner />
