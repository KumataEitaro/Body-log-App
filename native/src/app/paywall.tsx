...
// leadは「誰のためのプランか」の1行（社会的証明ではなく価値の言語化）。
// lead/featuresは関数にして呼び出し時にt()する（リテラルt('...')でi18n収集スクリプトに拾わせるため）
...
// ペイウォールに出すカード（PAYWALL_PLANSの順ではなくPLAN_INFOの並び＝安い順に上から）
...
export default function PaywallScreen() {
  ...
  // プレミアムカードの縁を「1本だけ」ゆっくり明滅させる（主役の視線誘導）。
  ...
  // 1枚のプランカード。isHero=プレミアム（グラデ縁＋バッジ）、それ以外は枠線のみの控えめ
  function renderCard(info: (typeof CARDS)[number]) {
    ...
    // このカードが選択中か。選択中でなければ価格ラジオは全部「未選択」で描く
    ...
    const body = (
      <>
        ...
        <Pressable
          ...
          style={({ pressed }) => [
            s.cta,
            ctaBuy && (isHero ? s.ctaHero : s.ctaOn),
            (isCurrent || opts.length === 0) && s.ctaOff,
            pressed && { opacity: 0.85 },
          ]}>
          <Text style={[s.ctaT, ctaBuy && s.ctaTOn]}>
            {isCurrent ? t('現在のプラン')
    ...
    return (
      <View key={info.plan} style={s.heroWrap}>
        ...
        <View style={s.badge}>
          <Sparkles size={12} color={C.panel} />
      ...
      <ScrollView contentInsetAdjustmentBehavior="automatic" contentContainerStyle={s.scroll}>
        ...
        {!purchasesAvailable() || (offers !== null && offers.length === 0) ? (
          ...
          <>
            ...
            {/* ライトは新規販売終了。すでに買っている人の画面には「取り上げられていない」ことを明示する */}
            {plan === 'lite' && (
...
const s = StyleSheet.create({
  ...
  heroGlow: { ...StyleSheet.absoluteFillObject, borderRadius: 18, backgroundColor: C.teal },
  ...
  badgeT: { color: C.panel, fontSize: 11, fontWeight: '800' },
  ...
  // CTA: 未選択カードは輪郭だけ（控えめ）、選択中だけ塗り。主役はアクセント塗り
  ...
  ctaHero: { backgroundColor: C.teal, borderColor: C.teal },
  ctaOff: { opacity: 0.5 },
  ctaT: { fontSize: 15, fontWeight: '700', color: C.teal },
  ctaTOn: { color: C.panel },
